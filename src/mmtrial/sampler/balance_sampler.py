...
class BalanceSampler(DefaultSampler):
    """Class Balanced Sampler.

    refer: https://github.com/KevinMusgrave/pytorch-metric-learning/blob/
        v2.6.0/src/pytorch_metric_learning/samplers/m_per_class_sampler.py


    Examples:
        train_dataloader = dict(
            xxxx,
            sampler=dict(type="BalanceSampler", num_per_class=4),
        )
    """

    def __init__(self, num_per_class: int, **kwargs) -> None:
        """Initialize the BalanceSampler.

        Args:
            num_per_class (int): number of samples per class in a batch
              At every iteration, this will return m samples per class. e.g.
              if dataloader's batchsize is 100, and num_per_class = 5, then 20
              classes with 5 samples each will be returned
    ...
    def calculate_num_iters(self) -> int:
    ...
    def get_labels_to_indices(self, labels) -> dict:
        """Get labels to indices.
